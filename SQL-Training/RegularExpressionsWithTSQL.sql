...
--2. Records with first character A and second characters L
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[A][L]%'
--3. Records with first character A and second characters L and third characters L
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[A][L][L]%'
--4. It starts character from A and D
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[A-D]%'
ORDER BY 1 
--5. The first character should be from A and D alphabets, The second character should be from F and L alphabet
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[A-D][F-I]%'
ORDER BY 1 
--6. Ending character should be from G and S
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '%[G-S]'
ORDER BY 1 
--7. Starting character should be A (first) and F (second),Ending character should be S
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[A][F]%[S]'
--8. We do not want the first character of output rows from A to T.
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[^A-T]%'
--9. The first character from R and S, then either P or I, any characters
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[R-S]%[P][I]%'
--10. We do not get case sensitive results
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[r-s]%[P][I]%'
--
...
select * from Characters 
where Alphabet COLLATE Latin1_General_BIN  like '[a]%'
--11. The first character should be uppercase character C,The second character should be lowercase character h
--Rest of the characters can be in any letter case
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] COLLATE Latin1_General_BIN LIKE '[C][h]%'
--12. Results with rows that contain number 0 to 9 in the beginning.
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
WHERE [Description] LIKE '[0-9]%'
--13. First digit from 1-5, the second digit should be in between 0 to 9
SELECT [Description]
FROM [AdventureWorks2019].[Production].[ProductDescription]
...
GO
--14. Identify valid email address from the user data
SELECT * 
FROM TSQLREGEX
...
VALUES ('1 The quick brown fox jumped over the lazy dogs.')
 ...
 , ('4')
 , ('-')
--1. How many rows have one alpha character?
 SELECT * 
 FROM TSQL_RegExTable
 WHERE RegExColumn LIKE '[a-z]'
--2. How many rows have only one alpha character in between h and j?
  SELECT * 
 FROM TSQL_RegExTable
 WHERE RegExColumn LIKE '[h-j]'
--3. How many rows start with any alpha character?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[a-z]%'
-- 4. How many rows have an alpha character somewhere in them?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '%[a-z]%'
--5. How many rows have the alpha character z somewhere in them?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '%[z]%'
--6. How many rows have do not start with an alpha character?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[^a-z]%'
--7. How many rows have only one numeric character?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[0-9]'
--8. How many rows have only one numeric character between 7 and 8?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[7-8]'
--9. How many rows start with any numeric character?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[0-9]%'
--10. How many rows have any numeric character in them?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '%[0-9]%'
--11. How many rows have do not start with a numeric character?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[^0-9]%'
--12. How many rows start with the character @ or -?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '[@-]%'
--13. How many rows have an @ or - character in them?
SELECT * 
FROM TSQL_RegExTable
WHERE RegExColumn LIKE '%[@-]%'
--14. How many rows have neither one alpha nor one numeric character?
SELECT * 
FROM TSQL_RegExTable
...
--15. How many SSN values of nine digits in a row?
SELECT *
FROM SSNTable
WHERE SSN LIKE '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]'
--16. How many SSNs with three digits, a dash, two digits, a dash, and four digits?
SELECT *
FROM SSNTable
WHERE SSN LIKE '[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]'
--17. How many TextFields with three digits, a dash, two digits, a dash, and four digits only?
SELECT *
FROM SSNTable
WHERE TextField LIKE '[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]'
--18.How many TextFields with three digits, a dash, two digits, a dash, and four digits somewhere in them?
SELECT *
FROM SSNTable
WHERE TextField LIKE '%[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]%'
--19. How many TextFields with three digits, a non digit, two digits, a non digit, and four digits somewhere in them?
SELECT *
FROM SSNTable
...
--1. Regex to Find Where Data is Only One Character and Value is from A to Z
SELECT *
FROM alphareg 
WHERE Alphabetic LIKE '[A-Z]'
--2. Regex to Find Where Data is Only One Character and Value is from C to D
SELECT *
FROM alphareg 
WHERE Alphabetic LIKE '[C-D]'
--3. Regex to Find Where Data is Two Characters and Values are from A to Z
SELECT *
FROM alphareg 
WHERE Alphabetic LIKE '[A-Z][A-Z]'
--4. Regex to Find Where Data is Any Length and First Character has a Value from A to Z
SELECT *
FROM alphareg 
WHERE Alphabetic LIKE '[A-Z]%'
--5. Fetching all the data rows of any character length that start with the alphabetic characters of B.
SELECT *
FROM alphareg 
WHERE Alphabetic LIKE '[B]%'
--6. Looking for data rows with 2 alphabetic characters in the range of A through D for each character.
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '[A-D][A-D]'
--7. First character being a T and the second character being either an A or W.
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '[T][AW]%' --[AW] means the “second character is either an A or W.” 
--8. Any characters are allowed at first (initial %),
...
--10. Any alphabetic combination in the first character [A-Z]
...
--11. Two of our data rows from the fourth query have “this” 
...
--13. Looking at our alphareg table to see if any rows start with any range of numbers 0 through 9
SELECT * 
FROM alphareg
WHERE Alphabetic   LIKE '[0-9]%'
--14. Looking for a data row that has a numerical character of any range anywhere in the table alphareg
SELECT * 
FROM alphareg
WHERE Alphabetic LIKE '%[0-9]%'
--15. Looking for values that only have two digits:
SELECT *
FROM alphanumreg
...
--16. How many rows of the decimal data start with either a two or a three?
SELECT *
FROM alphanumreg
WHERE NumData LIKE '[2-3]%'
--17. Complex Numeric Regex
...
--18. In first query below this, we want any number with two decimal places only for the AlphabeticNum column. 
SELECT *
FROM alphanumreg
WHERE AlphabeticNum LIKE '%[.][0-9][0-9]'
--19. In the second query we want the same except three decimal places only for the AlphabeticNum column. 
SELECT *
FROM alphanumreg
WHERE AlphabeticNum LIKE '%[.][0-9][0-9][0-9]'
--20. In the third query we want any number with at least two decimal places for the AlphabeticNum column.
SELECT *
FROM alphanumreg
WHERE AlphabeticNum LIKE '%[.][0-9][0-9]%'
--21. Look for negative values:
SELECT NumData
FROM alphanumreg
WHERE NumData LIKE '[-]%'
...
--22. Look for a specific pattern
SELECT AlphabeticNum
FROM alphanumreg
...
--23.Writing a regular expression query that looks for 3 first characters of any digit 0 through 9,
...
--24. Regex for Special Characters
--Looking for  data row with one special character of an exclamation point [!]
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '[!]'
--25. Looking for any special character of an exclamation point in any data row anywhere.
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '%[!]%'
--26. Getting  all the data rows that have punctuation characters
--in them staring with the most common of comma, period, exclamation point, question mark, semicolon and colon.
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '%[,.!?;:]%'
--27. How many sentences had a special character then a letter right after it (no spaces)?
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '%[,.!?;:][A-Z]%'
--28. Verifying if we have a special punctuation character, followed by a space,
--followed by any character, followed by an alphabetic character.
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '%[,.!?;:][ ]%[A-Z]%'
--29. Do any sentences have the special character of ” in our alphareg table
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '%[”]%'
...
--30. Finding all of our data rows from the alphareg table that started with any special character
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '[^A-Z]%'

--31. Finding any of our data rows from our alphanumreg table where the column AlphabeticNum started with a non-numerical character 
SELECT AlphabeticNum
FROM alphanumreg
WHERE AlphabeticNum LIKE '[^0-9]%'
--32. Excluding all the alphabetic and numerical character
SELECT *
FROM alphareg
WHERE Alphabetic LIKE '[^0-9a-z]%' 
--33. Looking for sentences that start with any alphabetic character, 
...
--34. In some situations, we may not want a space or another special character, 
...
SELECT *
FROM tbBizExamples
--35. Look for dates in the format starting with YYYYXMMXDD where X is a separator of some kind,
...
--36. Looking for a 16 digit credit card that begins with a number from 1 to 6 as a possibility 
...
--37. Looking at another possibility in these cases, by first ensuring the length is 19 using the LEN function 
...
--38. Look for SSN (Social Security Number) pattern:
SELECT *
FROM tbBizExamples
WHERE LEN(BizVarChar) = 11 
	AND BizVarChar LIKE '%[0-9][0-9][0-9][-][0-9][0-9][-][0-9][0-9][0-9][0-9]%'

--39. Looking for  Valid Phone Number Patterns with T-SQL
SELECT *
FROM tbBizExamples
...
--40. Looking for Valid URL Patterns with T-SQL
SELECT *
FROM tbBizExamples
WHERE BizVarchar LIKE '%[A-Z0-9][.][A-Z0-9]%[A-Z0-9][/][A-Z0-9]%'
--41. Looking for Valid Email Patterns with T-SQL
SELECT *
FROM tbBizExamples
